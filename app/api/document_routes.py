...
async def upload_document(
    ...
    try:
        ...
        document_id = await ingest_pdf(db=db, file_path=str(file_path))

        # Embedding generation and FAISS indexing happen asynchronously.
...
async def list_documents(
    ...
    page_size: int = Query(default=10, ge=1, le=100, description="Number of documents per page"),
    db: AsyncSession = Depends(get_db),
):
    documents, total = await get_documents_paginated(db=db, page=page, page_size=page_size)
...
async def get_document_details(
    ...
    document = await get_document_with_chunks(db=db, document_id=document_id)
    if document is None:
...
async def remove_document(
    ...
    deleted = await delete_document(db=db, document_id=document_id)
    if not deleted:
